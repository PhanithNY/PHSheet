//
//  PHContentInsetInteractionController.swift
//
//
...
public final class PHContentInsetInteractionController: NSObject, InteractionControlling {
  
  // MARK: - Properties
  
  public var interactionInProgress = false
  private weak var viewController: PHContentInsetPresentable!
  ...
  public init(viewController: PHContentInsetPresentable) {
    self.viewController = viewController
    super.init()
    
    if viewController.sheetDismissible {
      prepareGestureRecognizer(in: viewController.view)
      if let scrollView = viewController.sheetScrollView {
  ...
  private func prepareGestureRecognizer(in view: UIView) {
    let gesture = VerticalPanGestureRecognizer(target: self, action: #selector(handleGesture(_:)))
    view.addGestureRecognizer(gesture)
  }
  
  private func resolveScrollViewGestures(_ scrollView: UIScrollView) {
    let scrollGestureRecognizer = VerticalPanGestureRecognizer(target: self, action: #selector(handleGesture(_:)))
  ...
  func update(progress: CGFloat) {
    ...
    if let modalPresentationController = presentedViewController.presentationController as? PHContentInsetPresentationController {
  ...
  func finish(initialSpringVelocity: CGFloat) {
    ...
    let presentedViewController = transitionContext.viewController(forKey: .from) as! PHContentInsetPresentable
...
extension PHContentInsetInteractionController: UIGestureRecognizerDelegate {
